...
class Spot:
    ...
    def move_to_relative(self, dx, dy):
        """ Move by the specified relative coordinates, while keeping the same orientation."""
    ...
    def _get_current_position(self):
        ...
        local_x = -transform.position.x
        local_y = -transform.position.y
        
        yaw = math.radians(yaw)

        # Remove rotation of the robot from the poition
        global_x = local_x * math.cos(yaw) - local_y * math.sin(yaw)
        global_y = local_x * math.sin(yaw) + local_y * math.cos(yaw)

        return global_x, global_y, yaw
